...
Page({
  ...
  handleOrderNow() {
    ...
    if (warehousingCheckedList.length === 0) {
      ...
      wx.navigateTo({
        url: '/packageB/shippingInformation/index?yuycomm2='+that.data.yuycomm,
      })
    }
  },
  ...
  onLoad: function (options) {
    const that = this
		...
		that.setData({
			warehousingList,
      CustomBar,
      yuycomm:options.yuyIndex2
    })
    if(that.data.yuycomm==1){
      that.setData({
        selectionArr:'预约下单'
      })
    }else{
      that.setData({
		...
		console.log('warehousingList123', options)
  },
