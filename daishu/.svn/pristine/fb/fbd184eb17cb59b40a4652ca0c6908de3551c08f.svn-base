...
Page({
	...
	getRechargeList () {
		...
		app.http({
			...
			console.log(res, 'rechargeListRes')
			...
			console.log(err, 'rechargeListErr')
		})
	},
	...
	onShow () {
		const that = this
		wx.createSelectorQuery().selectAll('.nav-area').boundingClientRect(function (rects) {
			console.log('rects', rects)
			that.setData({
				navAreaHeight: rects[0].height
			})
		}).exec()
	},
    ...
    const that = this
		...
		const { nowTimeStamp, endTime } = that.data
  ...
  bindDateChange2(e) {
    const that = this
		...
		const { nowTimeStamp, todayStart, startTime } = that.data
