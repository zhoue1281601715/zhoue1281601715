...
Page({
	...
	consigneeAddress() {
		let isAdd = 1
		wx.navigateTo({
			url: '../addressManagement/index?isAdd=' + isAdd,
		})
	},
	// 将商品列表包装城接口需要的格式
	packagingData(arr) {
    ...
    let currentUser = this.data.currentUser
      ...
      destination = this.replaceProp()
      Object.assign(data, destination)
    }
		// return
		app.https({
			config: {
  ...
  navgateToPage(param) {
    ...
    //0:下单失败 1:下单成功
    wx.navigateTo({
      url: '../in_stock_res/in_stock_res?isOut=1&res=' + param,
    })
  },
	/*============================================
							 以下是基本操作 				
